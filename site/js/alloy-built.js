...
var createDataCollector = function createDataCollector(_ref) {
  ...
  var createEventHandler = function createEventHandler(options) {
    ...
    return lifecycle.onBeforeEvent(event, options, viewStart, documentUnloading).then(function () {
  ...
  return {
    lifecycle: {
      onBeforeEvent: function onBeforeEvent(event, options) {
        return Promise.resolve(options.eventMergeId).then(function (eventMergeId) {
...
if (namespaces) {
  namespaces.forEach(function (namespace) {
    ...
    var logger = createLogger(console$1, logController, "[" + namespace + "]");
